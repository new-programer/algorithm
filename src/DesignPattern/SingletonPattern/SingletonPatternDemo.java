...
/**
 * @Description 从 SingletonPattern 类获取唯一的对象。
 ...
 * @Author EricGao
 * @Date 21:42 2020/5/21
 */
public class SingletonPatternDemo {
    public static void main(String[] args) {
        //不合法的构造函数
        //编译时错误：构造函数 SingletonPattern() 是不可见的
        ...
        /*-------------------------2、懒汉式，线程安全--------------------------*/
        SingletonLazySecurity singletonLazySecurity = SingletonLazySecurity.getInstance();
        singletonLazySecurity.showMessage();

        /*-------------------------3、饿汉式--------------------------*/
        SingletonActive singletonActive = SingletonActive.getInstance();
        singletonActive.showMessage();

        /*-------------------------4、双检锁/双重校验锁（DCL，即 double-checked locking）--------------------------*/
        Singleton_DCL singleton_dcl = Singleton_DCL.getInstance();
        singleton_dcl.showMessage();

        /*-------------------------5、登记式/静态内部类--------------------------*/
        Singleton_SIC singleton_sic = Singleton_SIC.getInstance();
        singleton_sic.showMessage();
 ...
 * 实现难度：易
 *
 * 描述：这种方式是最基本的实现方式，这种实现最大的问题就是不支持多线程。因为没有加锁 synchronized，所以 严格 意义上它并不算单例模式。
...
class SingletonLazy{
    ...
    public void showMessage(){
        System.out.println("Hi, SingletonLazy");
    }
}
...
class SingletonLazySecurity{
    ...
    public void showMessage(){
        System.out.println("Hi, SingletonLazySecurity");
    }
}
 ...
 */
class SingletonActive{
    private static SingletonActive instance = new SingletonActive();
    private SingletonActive(){}
    public static SingletonActive getInstance(){
        return instance;
    }
    public void showMessage(){
        System.out.println("Hi, SingletonActive(即默认单例模式SingletonPattern)");
    }
}
...
class Singleton_DCL{
    ...
    public void showMessage(){
        System.out.println("Hi, Singleton_DCL");
    }
}


/**
 * 5、登记式/静态内部类(Registered / static internal class)
 * 是否 Lazy 初始化：是
 * 是否多线程安全：是
 * 实现难度：一般
 *
 ...
 * 第 3 种方式只要 SingletonActive 类被装载了，那么 instance 就会被实例化（没有达到 lazy loading 效果），
 * 而这种方式是 Singleton_SIC 类被装载了，instance 不一定被初始化。因为 SingletonHolder 类没有被主动使用，
 * 只有通过显式调用 getInstance 方法时，才会显式装载 SingletonHolder 类，从而实例化 instance。
 *
 * 想象一下，如果实例化 instance 很消耗资源，所以想让它延迟加载，另外一方面，又不希望在 Singleton_SIC 类加载时就实例化，
 * 因为不能确保 Singleton_SIC 类还可能在其他的地方被主动使用从而被加载，那么这个时候实例化 instance 显然是不合适的。
 * 这个时候，这种方式相比第 3 种方式就显得很合理。
 */
class Singleton_SIC{
    private static class SingletonHolder{
        private static final Singleton_SIC instance = new Singleton_SIC();
    }

    private Singleton_SIC(){}

    public static final Singleton_SIC getInstance(){
        return SingletonHolder.instance;
    }

    public void showMessage(){
        System.out.println("Hi, Singleton_SIC");
    }
}
...
enum SingletonEnum{
    instance;
    public void showMessage(){
        System.out.println("Hi, SingletonEnum");
    }
}